...
class AnomalyDetector:
    ...
    def __init__(self, model_dir="./models"):
        ...
        # Online retraining buffers / config
        self.max_buffer_size = int(os.getenv("ANOMALY_BUFFER_SIZE", "4000"))
        self.retrain_min_samples = int(os.getenv("ANOMALY_RETRAIN_MIN_SAMPLES", "200"))
        self.retrain_interval = timedelta(
            minutes=int(os.getenv("ANOMALY_RETRAIN_INTERVAL_MIN", "15"))
        ...
        self.last_retrain_at: datetime = datetime.min
    ...
    def train(self, training_data: List[dict], sensor_type: str = 'temperature'):
        try:
            ...
            if df.empty:
                logger.warning("No training samples available after filtering")
                return False
            ...
            try:
                ...
                logger.warning(f"Scaler fit failed, falling back to raw values: {scaler_err}")
                self.scaler = StandardScaler()
                X_scaled = X
    ...
    def predict(self, value: float, sensor_type: str = 'temperature') -> Tuple[float, bool]:
        try:
            self._record_sample(value, sensor_type)
            self._maybe_retrain()
            ...
            try:
                ...
                logger.debug(f"Scaler transform failed, using raw value: {scale_err}")
                features = np.array([[value]], dtype=float)

            prediction = self.isolation_forest.decision_function(features)
            anomaly_score = 1 - (prediction[0] + 1) / 2  # Normalize to 0-1
    ...
    def _record_sample(self, value: float, sensor_type: str):
        """Store latest reading for future retraining."""
        try:
            sample = {
                "value": float(value),
                "sensor_type": sensor_type,
            ...
            logger.debug(f"Failed to record sample for retraining: {err}")
    
    def _maybe_retrain(self):
        """Trigger periodic retraining when enough fresh data is available."""
        if len(self.training_buffer) < self.retrain_min_samples:
            return
        
        now = datetime.utcnow()
        if now - self.last_retrain_at < self.retrain_interval:
            return
        
        samples = list(self.training_buffer)
        success = self.train(samples, sensor_type=None)
        if success:
            logger.info(
                f"IsolationForest auto-retrained with {len(samples)} samples "
                f"(interval {self.retrain_interval}, min {self.retrain_min_samples})"
            )
            self.training_buffer.clear()
            self.last_retrain_at = now
        else:
            logger.warning("Auto-retraining skipped due to training failure")
    
    def force_retrain_from_buffer(self) -> bool:
        if not self.training_buffer:
            logger.info("No buffered samples available for manual retraining")
            return False
        samples = list(self.training_buffer)
        success = self.train(samples, sensor_type=None)
        if success:
