    ...
    try {
      ...
      const processed = items
        .map((item) => {
          ...
          const ts = new Date(item.createdAt || payload.ts || Date.now());
          if (isNaN(ts.getTime())) return null;

          return {
            time: ts.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }),
            temperature: Number(payload.temp ?? 0),
            humidity: Number(payload.humid ?? 0),
            gas: Number(payload.gas_ppm ?? 0),
      ...
      setChartError(error.message || 'Error');
      setChartData([]);
    } finally {
              ...
              <div style={styles.telemetry}>
                ...
                <div style={styles.telemetryGrid}>
                  ...
                  <div style={styles.telemetryItem}>
                    ...
                    <span style={styles.telemetryValue}>
                      {device.latestTelemetry.smoke || 'N/A'} ppm
                    </span>
                  </div>
                  <div style={styles.telemetryItem}>
